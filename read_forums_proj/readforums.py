...
for thread in thread_titles[1:]:
    ...
    thread_file = open(f"{Path.home()}\\small-projects-python\\read_forums_proj\\forums_thread_data\\Thread {thread_num}.txt", 'a')
...
directs = os.listdir(f"{Path.home()}\\small-projects-python\\read_forums_proj\\forums_thread_data")
...
for i in range(len(directs) - 1):
    file = open(f'{Path.home()}\\small-projects-python\\read_forums_proj\\forums_thread_data\\{directs_modified[i]}', 'r')
    ...
    contents.pop(1)
    file = open(f'{Path.home()}\\small-projects-python\\read_forums_proj\\forums_thread_data\\{directs_modified[i]}', 'w')
